...
import {DatepickerViewModel, STKDayTemplateData, STKMarkDisabled} from './datepicker-view-model';
import {Injectable} from '@angular/core';
import {isInteger, toInteger} from './utils/util';
...
export class StkDatepickerService {
  private _VALIDATORS:
      {[K in keyof DatepickerServiceInputs]: (v: DatepickerServiceInputs[K]) => Partial<DatepickerViewModel>} = {
        ...
        markDisabled: (markDisabled: STKMarkDisabled) => {
          if (this._state.markDisabled !== markDisabled) {
            return {markDisabled};
          }
        },
        maxDate: (date: StkDate) => {
          const maxDate = this.toValidDate(date, null);
          if (isChangedDate(this._state.maxDate, maxDate)) {
            return {maxDate};
          }
        },
        minDate: (date: StkDate) => {
  ...
  private _dateSelect$ = new Subject<StkDate>();
  ...
  get dateSelect$(): Observable<StkDate> { return this._dateSelect$.pipe(filter(date => date !== null)); }
  ...
  constructor(private _calendar: StkCalendar, private _i18n: StkDatepickerI18n) {}
  ...
  select(date: StkDate, options: {emitEvent?: boolean} = {}) {
  ...
  toValidDate(date: StkDateStruct, defaultValue?: StkDate): StkDate {
    const stkDate = StkDate.from(date);
    if (defaultValue === undefined) {
      defaultValue = this._calendar.getToday();
    }
    return this._calendar.isValid(stkDate) ? stkDate : defaultValue;
  }
